...
import { Calendar, User, Mail, Lock, CheckCircle, ArrowRight, Mic2, Building2, Star } from 'lucide-react';
...
const RegisterOrganizer: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await register(email, password, fullName, 'organizer');
      navigate('/dashboard');
    } catch (err: any) {
  ...
  return (
    <div className="min-h-screen flex bg-gradient-to-br from-violet-950 via-slate-900 to-fuchsia-950">
      {/* Left Side - Branding */}
      <div className="hidden lg:flex lg:w-1/2 items-center justify-center p-12 relative overflow-hidden">
        ...
        <div className="relative text-white max-w-md">
          ...
          <h2 className="text-4xl font-bold mb-6">Create Amazing Events</h2>
          <p className="text-xl text-violet-100 mb-8">
            Host virtual, hybrid, or in-person events. Manage registrations, engage attendees, and grow your community.
          </p>
          <div className="space-y-4">
            ...
            <div className="flex items-center space-x-3">
              <CheckCircle className="h-6 w-6 text-violet-400" />
              <span>Monetize with ticket sales</span>
            </div>
          </div>
        ...
        <Card className="w-full max-w-md shadow-2xl border-0 bg-slate-900/80 backdrop-blur-sm">
          <CardHeader className="space-y-1 text-center">
            <div className="lg:hidden flex items-center justify-center space-x-2 mb-4">
              <div className="w-10 h-10 bg-gradient-to-br from-violet-400 to-fuchsia-500 rounded-lg flex items-center justify-center">
                <Calendar className="h-6 w-6 text-white" />
              </div>
              <span className="text-2xl font-bold text-white">Convene</span>
            </div>
            <div className="inline-flex items-center justify-center space-x-2 px-3 py-1 bg-violet-500/20 rounded-full mb-2">
              <Building2 className="h-4 w-4 text-violet-400" />
              <span className="text-sm text-violet-300 font-medium">Organizer Account</span>
            </div>
            <CardTitle className="text-3xl font-bold text-white">Create an account</CardTitle>
            <CardDescription className="text-slate-400">
              Start hosting amazing events today
            </CardDescription>
          </CardHeader>
          <form onSubmit={handleSubmit}>
            ...
            <CardFooter className="flex flex-col space-y-4">
              <Button 
                type="submit" 
                className="w-full h-11 bg-gradient-to-r from-violet-600 to-fuchsia-600 hover:from-violet-700 hover:to-fuchsia-700 text-white font-semibold border-0"
                disabled={isLoading}
              >
                {isLoading ? (
